...
from utils.utils import benchmark, flops, str2bool, str2list
import models.deit
...
def get_args_parser():
    ...
    parser.add_argument('--model-ema', default=True, type=str2bool)
    ...
    parser.add_argument('--color_jitter', type=float, default=0.3, metavar='PCT', help='Color jitter factor (default: 0.4)')
    ...
    # * Mixup params
    parser.add_argument('--mixup', type=float, default=0.0, help='mixup alpha, mixup enabled if > 0. (default: 0.0)')
    parser.add_argument('--cutmix', type=float, default=0.0, help='cutmix alpha, cutmix enabled if > 0. (default: 0.0)')
    ...
    parser.add_argument('--dataset', default='IMNET', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19'], type=str, help='Image Net dataset path')
    ...
    parser.add_argument('--model_ema', default=False, type=str2bool, help='Enabling distributed evaluation')
    ...
    parser.add_argument('--use_bce',            default=False, type=str2bool, help='use bce loss for mixup or cutmix')
    ...
    parser.add_argument('--use_mctf',        default=False,   type=str2bool)
    parser.add_argument('--r_evals',         default=None,     type=str2list)
    parser.add_argument('--activate_layer',  default=[3,6,9], type=str2list)
    parser.add_argument('--mctf_type',       default=[0.35, 0.15, 0, 1, 1, 1, 20, 40, 1, 1, 0], type=str2list)
...
def main(args):
    ...
    model = create_model(
        ...
        # MCTF
        use_mctf       = args.use_mctf,
        mctf_type      = args.mctf_type,
        task_type      = args.task_type,
        activate_layer = args.activate_layer,
    )
    ...
    if utils.is_main_process():
        ...
        args.fv_flops = flops(model, (1, 3, 224, 224), device=device, round_num=3)
        print(f"Params : {args.n_parameters}M,  Flops : {args.fv_flops}G")
        # args.throughput = benchmark(
        #     model,
        #     device=device,
        #     verbose=True,
    ...
    if mixup_active:
        ...
        if args.use_bce:
            criterion = BinaryCrossEntropy(smoothing=args.smoothing, target_threshold=args.bce_target_thresh)
        else:
    ...
    if args.eval:
        log_stats = {"model": args.model, "resume": args.resume, 'mctf_type': args.mctf_type, 'activate_layer' : args.activate_layer,
                     "r": [], "throughput": [], "flops": [], "acc1": []}

        for r_eval in args.r_evals:
            print(f"---------------------------------- r_eval : {r_eval} ----------------------------------")
            log_stats["r"].append(r_eval)
            gflops = flops(model, (1, 3, 224, 224), r_eval=r_eval, round_num=4)
            log_stats["flops"].append(gflops)
            ...
            test_stats = evaluate(loader_val, model, device, r_eval)
            log_stats["acc1"].append(round(test_stats["acc1"], 3))
        Path(args.output_dir).mkdir(parents=True, exist_ok=True)
        with open(os.path.join("./result", "result.txt"), 'a') as f:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and utils.is_main_process():
            ...
            if args.model_ema: savepoint['state_dict_ema'] = get_state_dict(model_ema),
...
if __name__ == '__main__':
    ...
    if args.debug is not None:
        os.environ["CUDA_VISIBLE_DEVICES"] = str(args.debug)

    if args.pretrained:
        args.min_lr             = args.min_lr    * args.min_lr_times
        args.warmup_lr          = args.warmup_lr * args.min_lr_times
        args.lr                 = args.lr        * args.lr_times
        args.cooldown_epochs    = 10
        args.warmup_epochs      = 0

    if args.output_dir:
        args.output_dir = os.path.join(args.output_dir, args.wandb_project, args.wandb_version, args.wandb_name)

        resume_file = os.path.join(args.output_dir, "checkpoint.pth") if not args.resume else args.resume
