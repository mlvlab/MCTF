...
from utils.mctf import parse_r
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, input_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.mctf_type   = kwargs["mctf_type"]
        self.info = {"use_mctf"       : kwargs["use_mctf"],
                     "activate"       : kwargs["activate_layer"],
                     "r_student"      : self.mctf_type[0],
                     "r_teacher_max"  : self.mctf_type[1],
                     "trace_source"   : self.mctf_type[2],
                     "prop_attn"      : self.mctf_type[3],
                     "one_step_ahead" : self.mctf_type[4],
                     "tau_sim"        : self.mctf_type[5],
                     "tau_info"       : self.mctf_type[6],
                     "tau_size"       : self.mctf_type[7],
                     "bidirection"    : self.mctf_type[8],
                     "pooling_type"   : self.mctf_type[9],

                     "num_protected"  : self.cls_token_num + self.dist_token_num,
                     }
    ...
    def resetInfo(self, r_eval):
        self.info["teacher_source"]    = None
        self.info["student_source"]    = None
        self.info["teacher_size"]      = None
        self.info["student_size"]      = None
        self.info["r_eval"]            = r_eval
        self.info["r_teacher"] = 0 if not self.info["r_teacher_max"] else \
                                torch.randint(0, int(self.info["r_teacher_max"] * 100), size=(1,)) / 100 if self.info["r_teacher_max"] < 1 \
                                        else torch.randint(0, self.info["r_teacher_max"], size=(1,))

    def forward(self, x, r_eval = None):
        self.resetInfo(r_eval)
        result = {}
        ...
        for i, blk in enumerate(self.blocks):
            if run_teacher:  # TEACHER
                ...
                if i == len(self.blocks) - 1:
                    ...
                    x_teacher_cls = self.head(x_teacher[:, 0])
                    result["x_teacher"] = x_teacher_cls
